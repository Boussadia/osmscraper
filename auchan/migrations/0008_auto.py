...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        # Adding M2M table for field dalliz_category on 'Product'
        db.create_table('auchan_product_dalliz_category', (
        ...
        db.create_unique('auchan_product_dalliz_category', ['product_id', 'category_id'])

        # Adding M2M table for field tag on 'Product'
        db.create_table('auchan_product_tag', (
            ('id', models.AutoField(verbose_name='ID', primary_key=True, auto_created=True)),
            ('product', models.ForeignKey(orm['auchan.product'], null=False)),
            ('tag', models.ForeignKey(orm['auchan.tag'], null=False))
        ))
        db.create_unique('auchan_product_tag', ['product_id', 'tag_id'])


    def backwards(self, orm):
        # Removing M2M table for field dalliz_category on 'Product'
        db.delete_table('auchan_product_dalliz_category')
